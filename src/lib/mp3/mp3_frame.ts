...
export class MPEGFrameReader {
	...
	private readVbri(data: Buffer, frame: IMP3.RawFrame, offset: number): number {
		...
		offset += 4;
		const version = data.readIntBE(offset, 2, true);
		offset += 2;
		const delay = data.readIntBE(offset, 2, true);
		offset += 2;
		const quality = data.readIntBE(offset, 2, true);
		offset += 2;
		const bytes = data.readIntBE(offset, 4, true);
		offset += 4;
		const frames = data.readIntBE(offset, 4, true);
		offset += 4;
		const toc_entries = data.readIntBE(offset, 2, true);
		offset += 2;
		const toc_scale = data.readIntBE(offset, 2, true);
		offset += 2;
		const toc_entry_size = data.readIntBE(offset, 2, true);
		offset += 2;
		const toc_frames = data.readIntBE(offset, 2, true);
		offset += 2;
