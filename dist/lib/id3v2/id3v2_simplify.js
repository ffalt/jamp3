...
Object.keys(id3v2_frames_1.FrameDefs).forEach(key => {
    ...
    if (!slug) {
        console.log('DEVELOPER ERROR: Add a slug for the frame \'' + key + '\': \'' + id3v2_frames_1.FrameDefs[key].title.toLowerCase().replace(/ /g, '_') + '\',');
    }
});
function simplyfyFrame(frame) {
    ...
    const frameDef = id3v2_frames_1.matchFrame(frame.id);
...
exports.simplyfyFrame = simplyfyFrame;
